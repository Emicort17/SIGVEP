...
function EditProductModal({ isOpen, onClose, productData, onSuccess }) {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            category_id: productData?.category?.id_category || '',
        },
        ...
        onSubmit: async (values, { setSubmitting }) => {
            const confirmed = await alertaPregunta(
                ...
                "Esta acción actualizará la información del producto."
            );
            if (!confirmed) {
                setSubmitting(false);
                return;
            ...
            try {
                ...
                const response = await AxiosClient.put(`/productos/${productData.id_product}`, body);
                if (response?.data) {
                    alertaExito('Éxito', '¡Producto actualizado correctamente!');
