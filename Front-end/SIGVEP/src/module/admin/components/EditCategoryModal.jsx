...
function EditCategoryModal({ isOpen, onClose, categoryData, onSuccess }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            const confirmed = await alertaPregunta(
                ...
                "Esta acción actualizará la categoría."
            );
            if (!confirmed) {
                setSubmitting(false);
                return;
            }
            alertaCargando('Guardando categoría...', 'Por favor, espere');
            try {
                const body = {
                    name: values.name,
                    description: values.description,
                };
                const response = await AxiosClient.put(`/categorias/${categoryData.id_category}`, body);
                if (response?.data) {
                    alertaExito('Éxito', '¡Categoría actualizada correctamente!');
                    if (onSuccess) onSuccess(response.data);
                    onClose();
                }
            } catch (error) {
                const errorMessage = error.response?.data?.message || 'No se pudo actualizar la categoría';
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 sm:px-0" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
            <div className="relative bg-white rounded-lg shadow-lg w-full max-w-md mx-auto p-6" style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                ...
                <h2 className="text-2xl font-bold text-center mb-6 font-poppins">Editar Categoría</h2>
